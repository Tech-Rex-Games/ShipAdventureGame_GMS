/// @description  _ctb_word_to_message( text_area_width, text_area_height, words )
/// @param  text_area_width
/// @param  text_area_height
/// @param  words 
function _ctb_word_to_message() {
	/*
	...
	*/
	...
	for( i=0; i<ds_list_size( W ); i++ ) {
	    ...
	    if( is_string( w ) ) {
	        ...
	        ww = string_width( string_hash_to_newline(w) );
	        wh = string_height( string_hash_to_newline(w) );
	        if( wh > npy ) npy = wh;
                
	        if( px+ww < TW && py+wh < TH ) {
	            ds_list_add( Words,_ctb_create_word( px,py,current_color,current_font,w ) );
	            px += string_width( string_hash_to_newline(w) );
	        }
	        else if( py+npy < TH ) {
	            px = 0;
	            py += npy;
